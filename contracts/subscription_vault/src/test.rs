...
fn test_min_topup_below_threshold() {
    ...
    client.init(&token, &7, &admin, &min_topup, &43200);
    let id = client.create_subscription(&subscriber, &merchant, &10_000000i128, &(86400), &true, &None);
...
fn test_get_next_charge_info_contract_method() {
    ...
    // Create subscription
    let id = client.create_subscription(&subscriber, &merchant, &amount, &interval_seconds, &false, &None);
...
fn test_usage_enabled_with_different_amounts() {
    ...
    let id1 = client.create_subscription(&subscriber, &merchant, &100i128, &(24 * 60 * 60), &true, &None);
